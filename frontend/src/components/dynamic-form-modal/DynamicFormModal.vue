...
import {NCheckbox, NCheckboxGroup, NCollapse, NCollapseItem, type NForm, NInputNumber} from "naive-ui";
...
const {config = {}, cdss = false} = defineProps<{ config: FormData, cdss?: boolean }>()
...
const fieldTypeOptions = computed(() => {
  ...
  if (cdss) {
    options.push({label: '特殊值', value: 'radio'})
  }
  return options
})
// 添加新字段
...
<template>
  <n-modal v-model:show="showModal" preset="dialog" title="Dialog" style="width: 750px"
           :mask-closable="false">
    <template #header>
      <div>动态表单构建器</div>
    </template>
