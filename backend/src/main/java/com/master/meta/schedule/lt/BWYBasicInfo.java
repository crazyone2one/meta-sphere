...
//        List<Row> sensorList = sensorInRedis.stream().filter(row -> BooleanUtils.isFalse(row.getBoolean("deleted"))).toList();
        ...
        String fileName = projectNum + WkkSensorEnum.BWYDY.getKey() + DateFormatUtil.localDateTimeToString(now) + ".txt";
    ...
    private String bodyContent(List<Row> sensorInRedis, LocalDateTime now) {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            String deviceCode = projectNum + "BWY03" + randomString;
            content.append(deviceCode).append(";");
            content.append("BWY").append(randomString).append(";");
    ...
    public static JobKey getJobKey(String resourceId) {
        return new JobKey(resourceId, BWYBasicInfo.class.getName());
    }

    public static TriggerKey getTriggerKey(String resourceId) {
        return new TriggerKey(resourceId, BWYBasicInfo.class.getName());
    }
}
