...
public class KSWBasicInfo extends BaseScheduleJob {
    ...
    private KSWBasicInfo(SensorService sensorUtil, FileHelper fileHelper, FileTransferConfiguration fileTransferConfiguration) {
...
//        List<Row> sensorList = sensorInRedis.stream().filter(row -> BooleanUtils.isFalse(row.getBoolean("deleted"))).toList();
    ...
    private String bodyContent(List<Row> sensorInRedis, LocalDateTime now) {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            String deviceCode = projectNum + "KSW02" + randomString;
            content.append(deviceCode).append(";");
            content.append("KSW").append(randomString).append(";");
    ...
    public static JobKey getJobKey(String resourceId) {
        return new JobKey(resourceId, KSWBasicInfo.class.getName());
    }

    public static TriggerKey getTriggerKey(String resourceId) {
        return new TriggerKey(resourceId, KSWBasicInfo.class.getName());
    }
}
