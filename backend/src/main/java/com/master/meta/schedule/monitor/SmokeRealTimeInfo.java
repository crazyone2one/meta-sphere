...
public class SmokeRealTimeInfo extends BaseScheduleJob {
    ...
    private SmokeRealTimeInfo(SensorService sensorService, FileManager fileManager, FileTransferConfiguration fileTransferConfiguration) {
        super(sensorService, fileManager, fileTransferConfiguration);
    }

    @Override
    protected void businessExecute(JobExecutionContext context) {
        List<Row> sensorInRedis = sourceRows(SensorKGType.SENSOR_AQJK_1008.getKey(), SensorKGType.SENSOR_AQJK_1008.getTableName());
        List<Row> sensorList = sensorInRedis.stream()
                ...
                .filter(row -> {
                    ...
                    return SensorKGType.SENSOR_AQJK_1008.getSensorType().equals(sensorType);
                })
                .toList();
