...
public class CGKBasicInfo extends BaseScheduleJob {
    ...
    public CGKBasicInfo(SensorUtil sensorUtil) {
        this.sensorUtil = sensorUtil;
    }

    @Override
    protected void businessExecute(JobExecutionContext context) {
        List<Row> parsedObject = sensorUtil.getSensorFromRedis(super.projectNum, "CGK", "sf_shfz_cgk_cddy", false);
        LocalDateTime now = LocalDateTime.now(ZoneOffset.of("+8"));
        String fileName = super.projectNum + "_CGKCDDY_" + DateFormatUtil.localDateTimeToString(now) + ".txt";
